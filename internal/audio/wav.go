...
// SegmentPCM splits interleaved PCM into consecutive windows of at most
// segmentSamples samples, so a long recording can be transcribed one bounded
// chunk at a time instead of buffering/encoding the whole (multi-hour) track at
// once — which is what caused the worker to OOM. Each window after the first
// backs up by overlapSamples from the previous window's end, so a word spoken
// across a segment boundary appears in both segments and isn't lost or clipped
// (the small duplicated text is acceptable for summarization).
//
// The returned slices are sub-slices of the input backing array (no copy), so
// the caller must not mutate them. If segmentSamples <= 0 or the input is
// shorter than one segment, the whole input is returned as a single window.
// overlapSamples is clamped to [0, segmentSamples).
func SegmentPCM(pcm []int16, segmentSamples, overlapSamples int) [][]int16 {
	...
	for start < len(pcm) {
		...
		segs = append(segs, pcm[start:end])
		if end == len(pcm) {
			break
		}
