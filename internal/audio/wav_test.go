...
func TestSegmentPCM(t *testing.T) {
	...
	t.Run("empty input returns nil", func(t *testing.T) {
		if got := SegmentPCM(nil, 10, 2); got != nil {
			t.Fatalf("got %v, want nil", got)
		}
	})

	t.Run("segment size zero returns whole input", func(t *testing.T) {
		in := seq(10)
		got := SegmentPCM(in, 0, 2)
		if len(got) != 1 || len(got[0]) != 10 {
			t.Fatalf("got %d segments (first len %d), want 1 of len 10", len(got), lenOr0(got))
		}
	})

	t.Run("shorter than a segment returns whole input", func(t *testing.T) {
		in := seq(5)
		got := SegmentPCM(in, 10, 2)
		if len(got) != 1 || len(got[0]) != 5 {
			t.Fatalf("got %d segments, want 1 of len 5", len(got))
		}
	})

	t.Run("no overlap tiles exactly and covers all samples", func(t *testing.T) {
		in := seq(10)
		got := SegmentPCM(in, 4, 0)
		// 4 + 4 + 2
		if len(got) != 3 {
			t.Fatalf("got %d segments, want 3", len(got))
		}
		var recombined []int16
		for _, s := range got {
			recombined = append(recombined, s...)
		}
		for i, v := range recombined {
			if v != int16(i) {
				t.Fatalf("recombined[%d] = %d, want %d (no-overlap split must be exact)", i, v, i)
			}
		}
	})

	t.Run("first segment has no leading overlap", func(t *testing.T) {
		in := seq(20)
		got := SegmentPCM(in, 8, 3)
		if got[0][0] != 0 {
			t.Fatalf("first segment starts at %d, want 0", got[0][0])
		}
		if len(got[0]) != 8 {
			t.Fatalf("first segment len = %d, want 8", len(got[0]))
		}
	})

	t.Run("later segments include trailing overlap from prior window", func(t *testing.T) {
		in := seq(20)
		// step = 8 - 3 = 5; second window starts at 5, backs up by 3 => lo=2.
		got := SegmentPCM(in, 8, 3)
		if got[1][0] != 2 {
			t.Fatalf("second segment starts at %d, want 2 (3-sample overlap)", got[1][0])
		}
	})

	t.Run("every sample appears in at least one segment", func(t *testing.T) {
		in := seq(37)
		got := SegmentPCM(in, 8, 3)
		...
		for i, ok := range seen {
			if !ok {
				t.Fatalf("sample %d missing from all segments", i)
			}
		}
	})

	t.Run("overlap >= segment is clamped and terminates", func(t *testing.T) {
		in := seq(10)
		got := SegmentPCM(in, 4, 4) // clamped to 3 internally
		if len(got) == 0 {
			t.Fatalf("got 0 segments")
		}
		// Must terminate and cover the final sample.
		last := got[len(got)-1]
		if last[len(last)-1] != 9 {
			t.Fatalf("last segment ends at %d, want 9", last[len(last)-1])
		}
	})
}

func lenOr0(s [][]int16) int {
	if len(s) == 0 {
		return 0
	}
	return len(s[0])
