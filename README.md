...
- `/character add`, `edit`, `list`, `show`, and `delete` — `add`/`edit` open a structured fill-in form (name, class, race, level, bio); `add` prefills and re-saves your existing character; `show` displays a full sheet including the deeds recorded from approved `/review-session` proposals (defaults to your own character)
- `/world add`, `edit`, `list`, `show`, and `delete` for NPCs, locations, factions, quests, and story hooks — `add` opens a kind-specific form whose structured fields are stored as entity metadata and **adds** to any existing same-named entry (details accumulate, never overwritten); `edit` opens a pre-filled form that **replaces** the entry's fields; `delete` removes an entry (and drops it from `/ask`)
- `/world show name:<name>` and `/character show` are **read-only detail views**: `list` is an index whose rows are abbreviated, so `show` is how you read one entry in full (its description grows every time a proposal is approved, so long entries continue into follow-up messages rather than being cut). `kind` is optional on `/world show` — supply it only when the same name exists as more than one type. Both offer autocomplete over what's actually recorded.
