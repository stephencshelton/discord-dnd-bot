...
The bundled PostgreSQL uses the official [`pgvector/pgvector`](https://hub.docker.com/r/pgvector/pgvector) image, so the `vector` extension that `/ask` needs is always present (the app runs `CREATE EXTENSION IF NOT EXISTS vector` on startup). Its default service name/credentials match the default `config.database.url` (`discord-dnd-bot-postgresql`, db/user/password `discord_dnd_bot`). The bundled Redis uses the official `redis` image and its service name matches the default `config.redis.addr` (`discord-dnd-bot-redis-master:6379`). Override credentials under `postgresql.auth` / `redis.auth` and keep `config.database.url` / `config.redis.addr` (and `secrets.redisPassword` when Redis auth is on) in sync.
